...
- **🎨 赛博朋克主题** — Cyberpunk 2077 风格配色，管理面板内可视化换肤
...
| `Ctrl+Shift+G`（默认） | 打开管理面板（更新数据库 / 修改热键 / 查看日志 / 换肤） |
| 右键 | 清除所有标注和按钮 |
| ESC | 取消框选 |
...
├── core/
│   ├── constants.py         # 共享常量、ThemeConfig 单例、_ThemeProxy 属性代理
│   ├── overlay.py           # 全屏覆盖层（框选、标注、流式动画、DPI 适配）
│   ├── management_panel.py  # 管理面板（数据库更新、热键配置、主题换肤、日志）
│   ├── hotkey_config.py     # 热键配置读写
│   ├── stylesheet.py        # 动态 QSS 样式表生成
│   ├── fetch_worker.py      # 后台线程：从 GitHub 下载最新数据（7 步精细化进度）
│   └── update_worker.py     # 后台线程：执行数据库更新
├── recognizers/
│   └── relic_name.py        # 遗物名称 OCR 识别器（RapidOCR + 正则匹配 + 纠错）
├── data/
│   ├── relics.db            # SQLite 本地数据库（自动生成）
│   ├── theme.json           # 主题配色持久化文件
│   ├── hotkeys.json         # 热键配置持久化文件
│   ├── all.json             # WFInfo 全量掉落数据
│   ├── wfinfo_relics.py     # 数据库查询接口（四级匹配查找，线程安全）
...
| `ManagementPanel` | 独立管理窗口，六区：数据库状态/手动更新/自动更新/热键/主题换肤/日志 |
| `RelicNameRecognizer` | OCR 识别器，图像放大→RapidOCR→正则匹配→纠错 |
| `RelicDB` | SQLite 查询接口，四级匹配：别名→精确→模糊→正则纠错 |
| `ThemeConfig` | 主题单例，65 个配色字段，从 theme.json 加载覆盖 |
| `_ThemeProxy` | 属性代理，模块级常量自动跟随主题变化 |

### 线程安全

- **OCR 线程**：`QThread` + `finished` 信号自动清理，带超时强制终止
- **数据库**：`RelicDB` 使用 `threading.Lock` 双重检查锁定保护连接生命周期
...
```
ThemeConfig (单例) ─── data/theme.json (可选覆盖)
      │
      ├── 直接访问: theme.cyber_yellow → "#F7E300"
      │
      └── _ThemeProxy 代理 ──→ 模块级常量: CYBER_YELLOW, COLOR_VAULTED, ...
                                    │
                              import 到各模块，自动跟随主题变化
```

65 个配色字段，管理面板侧滑换肤面板可视化编辑，实时预览，增量持久化。
...
- **配置文件**：`data/theme.json`（不存在则使用默认赛博朋克主题）
- **修改方式**：管理面板 → 主题换肤 → 点击色块取色 → 实时预览 → 保存
- **重置**：删除 `data/theme.json` 恢复默认配色
