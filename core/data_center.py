    ...
    LINK_SECTIONS = [
        ...
        ("数据拉取", [
            ("⬇️", "下载 all.json", "GitHub Raw → HTTPS",
             ...
             True),
            ("⬇️", "下载 i18n.json", "GitHub Raw → HTTPS",
        ...
        ("数据解析", [
            ...
            ("🔗", "数据关联", "uniqueName 精确匹配",
             ...
             True),
            ("📊", "数据优先级", "三级合并策略",
             "zh_en_dict (16,907条) > all_items.json (15,729条) > i18n.json (补充)\n中文名: zh_en_dict > i18n > English fallback\n属性: all_items.json (category, tradable, rarity, ...)",
             True),
            ("🔗", "中英数据合并", "key 对齐",
             "dict.en + dict.zh 以 /Lotus/Language/... key 对齐合并\n统计: 中英皆有 / 仅英文 / 仅中文",
             True),
            ("✅", "双重交叉验证", "public-export-plus ↔ WFCD i18n",
             "以 dict.*.json 为主数据源，WFCD i18n.json 为补充源\n若 dict 缺失翻译 → 从 i18n 补充\n若 i18n 有独有 key → 追加到数据库",
             True),
            ("🏷️", "分类提取", "路径解析",
